...
const restoIcon = createIcon('#ff5f56', '🏪 Gacoan');
const pointIcon = (seq: number) => createIcon('#ffbd2e', `T${seq}`);
...
export default function RouteMap({ points }: RouteMapProps) {
  ...
  if (!mounted) return <div style={{ height: '300px', background: '#222', borderRadius: '16px', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>Loading Map...</div>;
  ...
  return (
    <div style={{ height: '300px', width: '100%', borderRadius: '16px', overflow: 'hidden', zIndex: 0, border: '1px solid var(--glass-border)' }}>
      <MapContainer center={[-7.055, 110.420]} zoom={14} style={{ height: '100%', width: '100%', zIndex: 0 }}>
        ...
        {/* Resto Marker */}
        <Marker position={restoCoord} icon={restoIcon}>
        ...
        {sortedPoints.map(p => {
          ...
          return (
            <Marker key={p.id} position={pointCoords[p.sequence]} icon={pointIcon(p.sequence)}>
